...
void run()
{
	...
	while (!(stabilizedFrame = stabilizedFrames->nextFrame()).empty())
	{
		...
		{
			if (!writer.isOpened())
				writer.open(outputPath, VideoWriter::fourcc('A', 'V', 'C', '1'),
...
void printHelp()
{
	cout << "OpenCV video stabilizer.\n"
		"Usage: Videostabilization  [arguments]\n\n"
		"Arguments:\n"
		"-i=,--input=input directory for videos. Default=Intput\n"
		...
		"  -o=, --output=input directory for videos. Default=Output\n"
...
int main(int argc, const char **argv)
{
	try
	{
		...
		string inputPath = arg("input");
		std::string folder(inputPath + "/*.*");
		...
		for (auto file : filenames) {
			...
			outputPath = arg("output") + "/" + "Stab_" + name + ".mov";//ext;

			cout << "Video in=" << file<<endl;
			cout << "Video out=" << outputPath << endl;

			Ptr<VideoFileSource> source = makePtr<VideoFileSource>(file);
